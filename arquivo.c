...
// saida: nenhum
void insereLivro(ARQ_BIN *b, LIVROS l)
{
    NO no;
    no.livro = l;
    no.dir = -1;
...
void imprimirArvorePreOrdem(ARQ_BIN b, int pos)
{
    NO no;
    carrega_no(&b, &no, pos);
...
NO *buscarPorTitulo(ARQ_BIN *b, char *titulo, int pos)
{
    ...
    if (!strcmp(titulo, no->livro.titulo)) {
        return no; // Retorne o nó encontrado
    }

    NO *resultado = NULL;

    // Buscar no lado esquerdo, se existir
    if (no->esq != -1) {
        resultado = buscarPorTitulo(b, titulo, no->esq);
    }

    // Se não encontrado à esquerda, busque no lado direito
    if (resultado == NULL && no->dir != -1) {
        resultado = buscarPorTitulo(b, titulo, no->dir);
    }

    free(no); // Liberar a memória alocada para `no`
    return resultado; // Retornar o resultado da busca
}
...
int remover_sub(ARQ_BIN *b, int cod, int reg)
{
    ...
    if (cod < no.livro.cod)
    {
        if (no.dir == -1 && no.esq == -1)
        {
            ...
            no.esq = remover_sub(b, cod, no.esq);
    }
    else
    {
        if (no.dir == -1 && no.esq == -1)
